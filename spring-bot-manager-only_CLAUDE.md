...
- This service uses **Flyway** — currently the only one of the three that
  does. P3 retargets these migrations at the Supabase `bot` schema
  (`spring.jpa.properties.hibernate.default_schema=bot`) — the migrations
  themselves carry over, just the connection target and schema change.
...
- **Target (ADR-001 as revised, A4)**: `smartbot` Postgres → `bot` schema on
  the shared Supabase project (P3), via `pg_dump`/`pg_restore` + Flyway
  retarget — not a data-model rewrite. Conversation/flow state stays in
  Redis (hot cache) with `bot_configs` / `conversation_state` (JSONB column
  for flow/variable state) tables in the `bot` schema. This is now a
  lower-effort consolidation step (1–2 weeks across all 3 services), not a
  3–5 week rewrite.
